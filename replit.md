...
- Impersonation: `attachUser` computes `req.auth` with both the real and effective identity. A SUPERADMIN can "enter" an org (acting org on the session) and acts with that org's effective role/org; `impersonating` is true. `requireRole` checks the EFFECTIVE role; `requireSuperadmin` checks the REAL role so impersonation can never grant console access. Enter/exit via `/platform/enter-org` and `/platform/exit-org`. A banner in PortalLayout shows the acting org with an "Exit to console" button.
...
- Platform/console endpoints (`/platform/*`) are guarded with `requireSuperadmin`: cross-org overview, org CRUD (create, edit name/region, enable/disable, cascade delete in a txn), org user CRUD, training module CRUD + reorder. Platform user `role` is constrained to the `OWNER|DRIVER` enum in the OpenAPI contract (validated by the generated Zod schema server-side).
...
- `requireSuperadmin` checks the REAL role; `requireRole` checks the EFFECTIVE role. Console/platform endpoints must use `requireSuperadmin` so an impersonating superadmin cannot reach the console with an org's effective role.
